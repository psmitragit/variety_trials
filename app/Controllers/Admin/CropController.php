...
class CropController extends BaseController
{
    public function index()
    {
        $admin = \auth_admin();
        $cropModel = new Crop();
        $crops = $cropModel->select('crops.*,GROUP_CONCAT(cv.name) as variables')
            ->join('crop_variables cv', 'crops.id=cv.crop_id', 'left');
        $admin['type'] == 1 && !empty($admin['crop']) ? $crops->whereIn('crops.id', \explode(',', $admin['crop'])) : "";
        $crops = $crops->where('crops.status', 1)->groupBy('crops.name')->find();
    ...
    public function variables($id = 0)
    {
        if ($this->request->isAJAX() && $this->request->is('post')) {
            $cropId = $this->request->getPost('id');
            $variables = Helpers::getVariables($cropId);
            $varieties = Helpers::getVarieties($cropId);
            $trials = Helpers::getTrials($cropId);
            return \response()->setJSON(['status' => true, 'variables' => $variables, 'varieties' => $varieties, 'trials' => $trials, 'hash' => \csrf_hash()]);
        } else {
            $variables = Helpers::getVariables($id);
    ...
    public function createSlug($name)
    {
        ...
        $crop = $cropModel->like('slug', $slug . '%')->find();
        foreach ($crop as $k => $l) {
            $slug .= '-' . $k + 1;
        }
        return $slug;
